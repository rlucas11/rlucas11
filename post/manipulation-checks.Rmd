...
link-citations: yes
thumbnailImage: http://res.cloudinary.com/demy2onqi/image/upload/g_center/v1493596082/ceceCouchEdit_z6fmry.jpg
thumbnailImagePosition: left
description: "How to deal with failed manipulation checks in replication studies."
draft: true
---
...
<br>
`r htmltools::HTML("{{< image classes=\"fancybox fig-50 center clear\" src=\"http://res.cloudinary.com/demy2onqi/image/upload/q_30/v1493596115/ceceCouch_ns54qn.jpg\"  title=\"Cece doesn't understand the rules of the couch.\" >}}")`
...
![Meta-Analysis](http://res.cloudinary.com/demy2onqi/image/upload/q_30/v1493598305/LSMeta_qrcqvo.jpg)
...
## Study 1
if (!file.exists('study1.csv')) {
    data <- GET('osf.io/qxbs2//?action=download',
                write_disk('study1.csv'))
}
s1 <- read.csv('study1.csv')

## Study 2
if (!file.exists('study2.csv')) {
    data <- GET('osf.io/gdpqm//?action=download',
                write_disk('study2.csv'))
}
s2 <- read.csv('study2.csv')


## Study 3
if (!file.exists('study3.csv')) {
    data <- GET('osf.io/drpkc//?action=download',
                write_disk('study3.csv'))
}
s3 <- read.csv('study3.csv')


## Study 4
if (!file.exists('study4.csv')) {
    data <- GET('osf.io/m59bx//?action=download',
                write_disk('study4.csv'))
}
s4 <- read.csv('study4.csv')


## Study 5
if (!file.exists('study5.csv')) {
    data <- GET('osf.io/n78r3//?action=download',
                write_disk('study5.csv'))
}
s5 <- read.csv('study5.csv')


## Study 6
if (!file.exists('study6.csv')) {
    data <- GET('osf.io/9bpuq//?action=download',
                write_disk('study6.csv'))
}
s6 <- read.csv('study6.csv')


## Study 7
if (!file.exists('study7.csv')) {
    data <- GET('osf.io/pzqj2//?action=download',
                write_disk('study7.csv'))
}
s7 <- read.csv('study7.csv')


## Study 8
if (!file.exists('study8.csv')) {
    data <- GET('osf.io/z5jgp//?action=download',
                write_disk('study8.csv'))
}
s8 <- read.csv('study8.csv')


## Study 9
if (!file.exists('study9.csv')) {
    data <- GET('osf.io/nr4ps//?action=download',
                write_disk('study9.csv'))
}
s9 <- read.csv('study9.csv')
...
> [We relied] on mental simulations of our own likely responses in setting up the procedures [@schwarz2003mood, p. 298]
...
It goes without saying that replication studies can only be interpreted if they are done well. Manipulation checks represent one way that the quality of the study and expertise of the investigators can be tested. However, a blanket rule that replications can be ignored---and perhaps even prevented from being published---if manipulation checks fail is problematic. In most cases, transparent reporting of all results is a better default. 
